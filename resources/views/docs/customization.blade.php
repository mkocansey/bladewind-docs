<x-app>
    ...
    <p>
        ...
        Once installed, all BladewindUI components exist in your project's <code class="inline">resources > views > components > <span class="text-red-400">bladewind</span></code> directory.
    ...
    <p>
        Using any of the BladewindUI components can be without the bladewind prefix. This <code class="inline text-red-400">&lt;x-bladewind.button&gt;Save User&lt;/x-bladewind.button&gt;</code> becomes <code class="inline text-red-400">&lt;x-button&gt;Save User&lt;/x-button&gt;</code>. Achieving this is actually quite easy.
    ...
    <p>Truely, you can! These components are in essence just Laravel blade templates that sit right in your project. If there are any implementations you are unhappy with, simply locate the particular blade template and dissect it at will.</p>
    <p>
    ...
    <p>
        Updates that touch the css and js files require the library's assets to be republished. Run the command below to republish the library's assets.
    </p>
     <pre class="lang-bash command-line"><code>php artisan vendor:publish --provider="Mkocansey\Bladewind\BladewindServiceProvider" --tag=bladewind-assets --force</code></pre><br />
     <p>
         <x-bladewind::alert type="info" show_close_icon="false">
